...
// Transmit Data 
  ...
  if(!answerRecieved1){
    if(!d1){
      Serial.println("Waiting for D1 to send Answer");
      d1 = true;
    }
    XBee.write(node1);
    nodeTalk = "1";
  ...
  if(answerRecieved1 && !answerRecieved2){
    if(!d2){
      Serial.println("Waiting for D2 to send Answer");
      d2 = true;
    }
    XBee.write(node2);
    nodeTalk = "2";
  ...
  if(answerRecieved1 && answerRecieved2){
    if(!d3){
      Serial.println("All answers recieved");
      Serial.println("Broadcasting correct answer");
      d3 = true;
    }
    XBee.write(answer);
    nodeTalk = "all";
...
//    Serial.print(rx);
//    Serial.print("|");
//    Serial.print((rx)&(0xC0));
//    Serial.print("|");
//    Serial.print((rx)&(0xC0) == 0);
//    Serial.println();
    if(((rx)&(0xC0)) == (0x00)){
      Serial.println("Recieved Answer");
      Serial.println("Analyzing Answer");
      if(nodeTalk=="1"){
        answer1 = ((0x0F)&(rx));
        if(!d4){
