...
//---------------------------------------------------------------------------------//
  ...
  // Read data as long as user has not submitted answer and the quiz has not ended
  if(!dataReady && startQuiz){
    ...
    if(!d1){
      Serial.println("Please select answer:");
      d1 = true;
    }
...
//  if( startQuiz && ( (millis() - startQuizTimer ) > 20000 ) ){
...
//    }
    ...
    // Correct answer from coordinator
        ...
        if(!d6){
          Serial.println("Correct Answer recieved from Coordinator. Comparing Answer .....");
          delayStart = millis(); 
          d6 = true;
