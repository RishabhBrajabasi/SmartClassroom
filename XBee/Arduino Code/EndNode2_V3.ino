...
byte tx = 0xdc;
byte seatOccupied = 0xfd;
byte seatUnoccupied = 0x01;
...
void loop() {
  ...
  if( submit == LOW && s == true){
    ...
    Serial.println();  
    Serial.println("");*/
    Serial.println("EndNode 2 to Coordinator: Sending 220");
    XBee.write(tx);
    delay(50);
  ...
  if(sitting == otherwise && sitting == 100){
    Serial.println("EndNode 2 to Coordinator: Sending 253");
    XBee.write(seatOccupied);
  }
  else if(sitting != otherwise && otherwise == 100){
    Serial.println("EndNode 2 to Coordinator: Sending 1");
  ...
  { 
    byte rx = XBee.read();
    Serial.print("Coordinator to End Node 2: Recieving ");
    Serial.print(rx);
    Serial.println();
  }
